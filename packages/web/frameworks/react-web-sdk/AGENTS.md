...
- Mount one owned stateful SDK root per browser runtime. `OptimizationProvider sdk={...}` injects
  the existing live singleton; it does not create a second stateful lifecycle. Read-only snapshot
