...
- [Preview support entry point](#preview-support-entry-point)
...
`@contentful/optimization-ios-bridge`, and is intended for any future SDK that surfaces a
...
The toolkit pairs with the [`registerPreviewPanel`](#registerpreviewpanel-preview-tooling-only)
bridge above: hosts call `registerPreviewPanel` with a plain object, pull the SDK signals out via
