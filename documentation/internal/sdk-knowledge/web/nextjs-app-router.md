...
- **`contentful?: ContentfulConfig` (managed fetching):** via the core `contentful` config. Lets the
  ...
  bound `OptimizationRoot` / `OptimizationProvider` also accept `prefetchManagedEntries` descriptors
  and `prefetchedManagedEntries` handoff entries; descriptors are fetched server-side through
  `sdk.prefetchManagedEntries()` and merged with any supplied handoff entries before passing
  `prefetchedManagedEntries` to React Web. The factory strips `contentful` before building the
  CLIENT provider config (`toClientProviderConfig` destructures `contentful: _contentful`); managed
  fetching runs on the server side. source:
  `packages/web/frameworks/nextjs-sdk/src/app-router-server.tsx:114-167,223-240,303-319`;
  `packages/web/frameworks/nextjs-sdk/src/bound-component-types.ts:48-64,80-84`;
  `packages/universal/core-sdk/src/CoreBase.ts` `CoreConfig.contentful`, `fetchOptimizedEntry`,
...
| ---------------------------------------------------------------------------------------- | --------- | --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OptimizationRoot`                                                                       | component | `/app-router` (bound) | `children`; `prefetchManagedEntries?`; `prefetchedManagedEntries?`                                                                                                     | element                                   | `packages/web/frameworks/nextjs-sdk/src/app-router-server.tsx:114-133,156-167`; `packages/web/frameworks/nextjs-sdk/src/bound-component-types.ts:80-84` |
| `OptimizationProvider`                                                                   | component | `/app-router` (bound) | `children`; `prefetchManagedEntries?`; `prefetchedManagedEntries?`                                                                                                     | element / `null`                          | `packages/web/frameworks/nextjs-sdk/src/app-router-server.tsx:135-154,156-167`; `packages/web/frameworks/nextjs-sdk/src/bound-component-types.ts:80-84` |
| `OptimizedEntry`                                                                         | component | `/app-router` (bound) | discriminated union `baselineEntry` (manual) XOR `entryId` (+`entryQuery?`, managed server fetch); render-prop child. Omits per-render `liveUpdates`/`loadingFallback` | element / `null`                          | `packages/web/frameworks/nextjs-sdk/src/app-router-server.tsx:58-65,169-240`                                                                            |
