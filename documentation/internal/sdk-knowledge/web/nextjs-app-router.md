...
- API clients default an omitted `environment` to `master`; an explicitly configured environment is
  used as supplied. It is the Contentful space environment used by the Experience and Insights APIs.
  Guides should still show the value when the reader must make project targeting unambiguous.
