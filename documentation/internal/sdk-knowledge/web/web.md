...
- Config keys (verified):
  - `spaceId` and `environment` (optional, default `'master'`, Contentful space environment used by
    the Experience and Insights APIs).
    source: api-client#ApiClientBase.ts#spaceId; api-client#ApiClientBase.ts#environment; api-client#ApiClientBase.ts#DEFAULT_ENVIRONMENT
