...
- **Server:** `bindNextjsPagesRouterServerOptimization(config)` → `{ createRequestHandoff }`. Server
  ...
  - **`prefetchManagedEntries` option:** the server prefetches ID or content-type/slug descriptors
    after request data loads and appends their handoffs to any supplied `handoff.entries`. Slug
