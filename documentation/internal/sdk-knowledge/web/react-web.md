...
- `OptimizationRoot` emits or skips a handoff-owned initial page event only when both `routeKey` and
  `buildPagePayload` are supplied. If `handoff.initialPageEvent` is `'emit'` without those inputs, it
  warns and skips the browser initial page event.
