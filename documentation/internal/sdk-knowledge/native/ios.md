...
- Preview panel: `PreviewPanelOverlay` (SwiftUI FAB + sheet) and `PreviewPanelViewController`
  ...
  app-supplied `PreviewContentfulClient` (100-entry paginated; `ContentfulSDKPreviewClient` wraps an
  existing `contentful.swift` client, `ContentfulHTTPPreviewClient` calls the CDA directly), then
