...
| State / result types                   | `OptimizationState`, `ResolvedOptimizedEntry`, `CTEntry`, `PreviewState` (+ DTOs), `JSONValue`, `OptimizationError`                                                           | extern:packages/ios/ContentfulOptimization/Sources/ContentfulOptimization/Core/OptimizationState.swift; extern:packages/ios/ContentfulOptimization/Sources/ContentfulOptimization/Core/ResolvedOptimizedEntry.swift; extern:packages/ios/ContentfulOptimization/Sources/ContentfulOptimization/Contentful/CTEntry.swift; extern:packages/ios/ContentfulOptimization/Sources/ContentfulOptimization/Core/OptimizationError.swift        |
| `contentful.swift` adapter             | `CTEntry` bridges `Contentful.Entry` and the resolver's entry JSON                                                                                                            | extern:packages/ios/ContentfulOptimization/Sources/ContentfulOptimization/Contentful/CTEntry.swift                                                                                                                                                                                                                                                                                                                                     |
...
| Preview panel                          | `PreviewPanelOverlay` (SwiftUI), `PreviewPanelViewController` (UIKit), `PreviewPanelConfig`, `PreviewContentfulClient` / `ContentfulHTTPPreviewClient`, `PreviewPanelContent` | extern:packages/ios/ContentfulOptimization/Sources/ContentfulOptimization/Preview/PreviewPanelViewController.swift#PreviewPanelViewController; extern:packages/ios/ContentfulOptimization/Sources/ContentfulOptimization/Preview/PreviewContentfulClient.swift#ContentfulHTTPPreviewClient; extern:packages/ios/ContentfulOptimization/Sources/ContentfulOptimization/Preview/PreviewContentfulClient.swift#ContentfulSDKPreviewClient |
...
- `OptimizationConfig`: `spaceId` and `clientId` required; `environment` (Ninetailed/Optimization
  environment, used by the Insights API) has a **Swift-side default `"main"`** in the initializer
  (unlike the JS SDKs, which fall back to the api-client `DEFAULT_ENVIRONMENT`); `contentfulEnvironment`
  (Contentful space environment, used by the Experience API) has a Swift-side default `"master"`;
  ...
  `onEventBlocked` all optional. source: extern:environment default "main", contentfulEnvironment default "master", logLevel default .error, spaceId and clientId required — packages/ios/ContentfulOptimization/Sources/ContentfulOptimization/Core/OptimizationConfig.swift#OptimizationConfig
...
- Single-locale CDA contract and include-depth requirement are shared and app-owned: fetch with one
  ...
  source: core-sdk#resolvers/OptimizedEntryResolver.ts#OptimizedEntryResolver; core-sdk#contentful/OptimizedEntry.ts#OptimizedEntryFields; concept:entry-personalization-and-variant-resolution; kb:shared/concepts.md
...
- `contentful.swift` integration: `CTEntry` converts a `Contentful.Entry` into the
  `{sys, fields, metadata}` shape the resolver expects, recursively expanding resolved links,
  assets, and Rich Text while emitting an empty metadata object when the source has none.
  `OptimizedEntry(entry: Contentful.Entry, ...)` performs this conversion once and returns resolved
  content through `CTEntry` instead of a raw dictionary.
  source: extern:packages/ios/ContentfulOptimization/Sources/ContentfulOptimization/Contentful/CTEntry.swift; extern:packages/ios/ContentfulOptimization/Sources/ContentfulOptimization/Views/OptimizedEntry.swift
- The imperative UIKit path has the same `Contentful.Entry` overload as the SwiftUI view:
  `OptimizationClient.resolveOptimizedEntry(baseline: Contentful.Entry, selectedOptimizations:)`
  maps `baseline` through `CTEntry` once, then delegates to the dictionary-based overload and wraps
  its result in `ResolvedOptimizedEntry` (`entry: CTEntry` instead of a raw dictionary). It inherits
  the dictionary-based overload's fail-soft behavior exactly — not
  initialized, a serialization error, or an unparseable bridge result all fall back to the mapped
  baseline with `selectedOptimization`/`optimizationContextId` nil.
  source: extern:packages/ios/ContentfulOptimization/Sources/ContentfulOptimization/Core/OptimizationClient.swift; extern:packages/ios/ContentfulOptimization/Sources/ContentfulOptimization/Core/ResolvedOptimizedEntry.swift
- `CTEntry.contentTypeId` reads `sys.contentType.sys.id` and returns `nil` when that path is absent,
...
- Preview panel: `PreviewPanelOverlay` (SwiftUI FAB + sheet) and `PreviewPanelViewController`
  ...
  app-supplied `PreviewContentfulClient` (100-entry paginated; `ContentfulHTTPPreviewClient` calls
  the CDA directly). `PreviewPanelConfig`, `PreviewPanelOverlay`, `PreviewPanelViewController`, and
  `addFloatingButton` each also overload on a raw `Contentful.Client`, wrapping it in the internal
  `ContentfulSDKPreviewClient` so an app that already reads `contentful.swift` shares one client and
  implements no entry mapping. The panel then calls `loadDefinitions` so the bridge bakes a preview
  model. Opening/closing the panel toggles
