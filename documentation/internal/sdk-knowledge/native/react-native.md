...
- Managed entry fetching through the app-owned `contentful.js` client supports the flat
  ...
  shared query, cache, failure, and handoff behavior.
...
| `OptimizedEntry`                       | component | root        | Manual `baselineEntry`, flat `entryId` + `entryQuery`, or object descriptor under `managedEntry`                            | `View` element, or `null`/fallback                                                                                                                   | react-native-sdk#components/OptimizedEntry.tsx#OptimizedEntry; react-native-sdk#components/OptimizedEntry.tsx#OptimizedEntryProps                                                                       |
...
| `useViewportTracking`                  | hook      | root        | `{ entry, selectedOptimization?, optimizationContextId?, enabled? }`                                                        | `{ isVisible, onLayout }`                                                                                                                            | react-native-sdk#hooks/useViewportTracking.ts#useViewportTracking; react-native-sdk#hooks/useViewportTracking.ts#UseViewportTrackingOptions                                                             |
...
- Entry source (managed or manual): `OptimizedEntry` / `useOptimizedEntry` use `baselineEntry` for
  an app-fetched entry, use `entryId` + `entryQuery` for the flat managed ID path, and put
  object descriptors under `managedEntry`. A slug descriptor's field defaults to `slug`; source
  changes refetch stale-safely, and successful metadata, callbacks, and interaction tracking use
  the fetched entry's `sys.id`. Managed fetch is driven by
...
- Include-depth requirement (resolver-driven): the resolver reads SDK-fixed Optimization fields off
  ...
  source: core-sdk#resolvers/OptimizedEntryResolver.ts#OptimizedEntryResolver; core-sdk#contentful/OptimizedEntry.ts#OptimizedEntryFields; core-sdk#contentful/OptimizationEntry.ts#OptimizationEntryFields
...
- Managed-fetch failure: when an ID or slug source rejects, the source
