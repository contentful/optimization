...
| Contentful entry fetching and locale shape       | Required for first integration | Establish the app-owned single-locale fetch that feeds resolution.    | Distinguish an existing ID fetch from an optional route-slug query through the app's existing Contentful client; do not invent an SDK or app protocol; exact content-type/slug-field equality filters; concrete locale, include, and `limit=2`; zero/duplicate handling; pass the one fetched `CDAEntry` directly to Compose or convert it to a map for imperative resolution; preserve the raw-map path; no native managed fetch; SDK locale versus CDA locale. | [render](../../internal/sdk-knowledge/native/android.md#render--entry-resolution), [identifiers](../../internal/sdk-knowledge/native/android.md#identifier-ownership)      |
| Entry resolution and fallback rendering          | Required for first integration | Complete the entry half of Milestone 1 through `OptimizedEntry`.      | Render-lambda example; direct `CDAEntry` Compose overload; `CTEntry.from(entry).toMap()` conversion before the map-only imperative `resolveOptimizedEntry`; preserved raw-map path; baseline fallback; loading treatment.                                                                                                                                                                                                                                        | [render](../../internal/sdk-knowledge/native/android.md#render--entry-resolution), [fallback](../../internal/sdk-knowledge/native/android.md#failure--fallback-behavior)   |
...
| Entry interaction tracking                       | Common but policy-dependent    | Explain viewport-based views and taps and their opt-outs.             | `OptimizationLazyColumn`; fixed view timing and threshold; tap versus app click; `trackViews`/`trackTaps` opt-outs; `onTap` baseline note.                                                                                                                                                                                                                                                                                                                       | [events](../../internal/sdk-knowledge/native/android.md#events--tracking)                                                                                                  |
...
| Strict event policy and queue controls           | Advanced or production-only    | Expose production privacy and delivery posture.                       | `allowedEventTypes = emptyList()`; `onEventBlocked`/`blockedEventStream` proof; queue policy; blocked not replayed.                                                                                                                                                                                                                                                                                                                                              | [consent](../../internal/sdk-knowledge/native/android.md#consent--persistence), [setup](../../internal/sdk-knowledge/native/android.md#setup--initialization-and-binding)  |
...
- Omit the personalization-explainer managed-fetch clause: the Android SDK has no native managed
  fetch; the app owns its existing ID lookup or optional content-type/slug query and passes the
  fetched entry to resolution. Facts:
  [render](../../internal/sdk-knowledge/native/android.md#render--entry-resolution).
- Prove "init plus one accepted screen event" rather than an entry render in the quick start: a
  native entry-render proof would force an app-specific Kotlin CDA fetch into the
...
- Keep the dependency/build step inline in the quick start: add the Maven Central dependency and run a
  ...
  [package](../../internal/sdk-knowledge/native/android.md#package--entry-points).
- In Before you start, require a working app-owned Contentful Delivery API client or fetch layer,
  not credentials alone. The Core entry path depends on a fetched entry and the native SDK does not
  supply that fetcher. Facts:
  [render](../../internal/sdk-knowledge/native/android.md#render--entry-resolution).
...
| ------------------------------------- | ----------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `No OptimizationClient provided`      | Calling SDK helpers outside `OptimizationRoot`.                   | [setup](../../internal/sdk-knowledge/native/android.md#setup--initialization-and-binding)                                                                         |
