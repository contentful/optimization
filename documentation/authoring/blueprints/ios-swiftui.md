...
| Contentful entry fetching and locale shape | Required for first integration | Establish the app-owned single-locale fetch that feeds resolution.    | Distinguish an existing ID fetch from an optional route-slug query through the app's existing Contentful client; do not invent an SDK or app protocol; exact content-type/slug-field equality filters; concrete locale, include, and `limit=2`; zero/duplicate handling; pass the one fetched entry to native resolution; no native managed fetch; SDK locale versus CDA locale. | [render](../../internal/sdk-knowledge/native/ios.md#render--entry-resolution), [identifiers](../../internal/sdk-knowledge/native/ios.md#identifier-ownership)      |
| Entry resolution and fallback rendering    | Required for first integration | Complete the entry half of Milestone 1 through `OptimizedEntry`.      | `contentTypeId`/`hasField` narrowing; direct resolver; fallback; loading.                                                                                                                                                                                                                                                                                                        | [render](../../internal/sdk-knowledge/native/ios.md#render--entry-resolution), [fallback](../../internal/sdk-knowledge/native/ios.md#failure--fallback-behavior)   |
...
| Entry interaction tracking                 | Common but policy-dependent    | Explain viewport-based views and taps and their opt-outs.             | Scroll provider; fixed view timing and threshold; tap versus SwiftUI `Button`; `trackViews`/`trackTaps` opt-outs; `onTap` gating.                                                                                                                                                                                                                                                | [events](../../internal/sdk-knowledge/native/ios.md#events--tracking)                                                                                              |
...
- Omit the personalization-explainer managed-fetch clause: the iOS SDK has no native managed fetch;
  the app owns its existing ID lookup or optional content-type/slug query and passes the fetched
  entry to resolution. Facts:
  [render](../../internal/sdk-knowledge/native/ios.md#render--entry-resolution).
- Prove "init plus one accepted screen event" rather than an entry render in the quick start: a
  native entry-render proof would force an app-specific Swift CDA fetch into the
