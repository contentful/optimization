...
`@contentful/optimization-nextjs/pages-router/server` for `getServerSideProps`. Manual `/server`,
`/client`, `/request-handler`, `/esr`, and `/tracking-attributes` helpers remain available for
lower-level control. Non-Next.js server-rendered apps can combine `@contentful/optimization-node` on
the server with `@contentful/optimization-web` or `@contentful/optimization-react-web` in the
browser.
