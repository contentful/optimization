...
entries in Server Components before HTML leaves the server, then hydrates a browser SDK for page
events, entry interaction tracking, consent controls, identify, and reset.
...
- [Core integration](#core-integration)
  ...
  - [Hydrated client provider and route tracking](#hydrated-client-provider-and-route-tracking)
...
| Optimization client ID and environment                             | Required for first integration | Yes                      | Server SDK config and `OptimizationRoot` props for hydrated client integrations      |
...
| Hydrated `OptimizationRoot` and route tracker                      | Required for first integration | No                       | App Router layout                                                                    |
| Server request consent policy                                      | Common but policy-dependent    | Yes                      | Server calls, proxy consent resolver, browser controls, CMP, or account controls     |
| Profile persistence and anonymous ID cookie continuity             | Common but policy-dependent    | No                       | Request handler cookie options and browser-readable `ctfl-opt-aid`                   |
...
request handler binds Next.js request and response objects.
...
| `@contentful/optimization-nextjs/request-handler`     | Next.js proxy or middleware                    | Request context, server page event calls, and anonymous ID cookie persistence  |
| `@contentful/optimization-nextjs/client`              | Client Components and hydrated layout children | React provider, hooks, App Router page tracker, and entry interaction tracking |
...
3. Reuse the same singleton from Server Components and from the proxy or middleware handler.
...
The request handler reads the incoming request, calls the request-bound `page()` flow when
configured to do so, and writes the adapter's default anonymous ID cookie, `ctfl-opt-aid`, to the
response when persistence consent permits it. Server Components read cookies from the incoming
request; response cookies written by proxy are for subsequent browser requests and hydrated browser
continuity.

1. Use `resolveConsent` to map your application policy to SDK request consent.
2. Use `getLocale` when the Experience API locale and event context must match the application
   content locale.
3. Leave the anonymous ID cookie browser-readable when the hydrated client must continue the same
   profile. Do not mark it `HttpOnly`.
4. Use `shouldRequestOptimization` when consent policy must prevent server-side profile-producing
   calls before opt-in.
...
export const proxy = createNextjsOptimizationRequestHandler(optimization, {
  ...
  // Resolve the same app-owned consent record that browser controls update.
  resolveConsent: ({ request }) => getAppConsent(request),
  // Skip profile-producing server calls before event consent is granted.
  shouldRequestOptimization: ({ consent }) =>
    typeof consent === 'object' && consent.events === true,
})
```

Use accepted consent directly only when your application policy permits default-on startup:

**Copy this:**

```ts
// Accepted startup allows the request handler to emit page events and persist continuity.
...
### Hydrated client provider and route tracking

**Integration category:** Required for first integration

SSR content rendering does not need browser JavaScript, but page tracking, entry interactions,
consent controls, identify, and reset run after hydration through the Next.js client entry.
...
4. Set `initialPageEvent="skip"` when the server already emitted the page event for the initial
...
<OptimizationRoot
  ...
  // Accepted browser startup enables hydrated route events.
...
The hydrated client can automatically observe server-rendered entry wrappers when the markup
contains the `data-ctfl-*` tracking attributes. Use `ServerOptimizedEntry` to render those
attributes from the same baseline entry and resolved data used for SSR content.
...
of these paths before depending on non-sticky entry views, clicks, hovers, or flag views:
`defaults.profile`, a persisted browser profile loaded under persistence consent, or a later browser
Experience call such as `page()`, `identify()`, `track()`, or sticky `trackView()`. A readable
`ctfl-opt-aid` cookie alone does not populate the current browser profile signal. Sticky
`trackView()` can bootstrap through Experience before sending its paired Insights event.
...
2. Read hydrated activity from `states.eventStream`.
...
hydrated provider. The SDK does not choose Contentful locales or modify CDA requests for you.
...
5. Use `initialPageEvent="skip"` for the first hydrated route when a server page call already
   emitted the initial page event.
...
more than immediate post-hydration content changes.
...
- **Consent behavior** - Default-on startup is backed by application policy, or proxy, Server
  Components, browser controls, and cookies all read the same consent source.
...
| Anonymous profile continuity is lost                         | The anonymous ID cookie is absent, `HttpOnly`, denied by persistence consent, or cleared on reset | Inspect `ctfl-opt-aid`, request-handler cookie options, browser consent state, and withdrawal logic        |
...
- [`implementations/nextjs-sdk_ssr`](../../implementations/nextjs-sdk_ssr/README.md) - Working
  ...
  Use it to compare proxy consent gating, server entry resolution, `ServerOptimizedEntry`, App
  Router layout tracking, and browser controls.
