...
1. Create a `PreviewContentfulClient` for the space and environment holding your Optimization
   entries. Wrap an existing `contentful.swift` client with `ContentfulSDKPreviewClient(client:)`, or
   use the built-in `ContentfulHTTPPreviewClient` when the app has no Contentful client to share.
   Either one fetches the `nt_audience` and `nt_experience` definitions.
