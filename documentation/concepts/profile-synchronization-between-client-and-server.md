...
| **Next.js adapter**              | Hybrid runtime. The App Router server binding's request family creates and supplies one browser `handoff` for its request components. Pages Router, manual server, and edge helpers return explicit browser handoffs. Response-capable request handlers, Pages Router, edge, and lower-level response-persistence helpers write or clear `ctfl-opt-aid`. Client components continue through the Web or React Web SDK state. |
...
- **Configured defaults** - `defaults.profile`, `defaults.selectedOptimizations`,
  ...
  response. In React Web provider handoff and manual Next.js provider or root handoff, keep
  `defaults` for configuration or default state such as consent policy, and pass server/static/edge
  browser state through the `handoff` prop when the provider or root receives that state directly.
  In the App Router server binding's nested `request` family, the request root or provider receives
  the SDK-created handoff automatically. Top-level App Router server components still accept an
  explicit handoff for static, public-permutation, analytics-only, or advanced manual flows. In
  Pages Router, pass `pageProps.contentfulOptimization.handoff` from `getServerSideProps` into the
  bound root or provider in `pages/_app.tsx`.
...
whether the browser needs only the shared profile ID or bootstrapped browser state.
...
| **Server bootstraps the browser** | The client must continue from the same evaluated data before its first browser Experience response.                              | For direct Web SDK initialization, serialize the server's `profile`, `selectedOptimizations`, and `changes` into `defaults`. For private App Router rendering, use the server binding's nested request root or provider, which creates and supplies the request handoff. Pass explicit handoffs only to top-level App Router static, public-permutation, analytics-only, or advanced manual roots and providers. For Pages Router, pass `pageProps.contentfulOptimization.handoff` through the bound root or provider in `_app.tsx`. For React Web and manual Next.js provider or root handoff, pass a browser handoff built from the server/static/edge state through the root or provider. |
...
SDK initialization, use explicit defaults. The App Router server binding's request root and provider
receive the SDK-created request handoff automatically. Top-level App Router components receive an
explicit selection or manual handoff when that route owns the state. Pages Router bound components
receive `pageProps.contentfulOptimization.handoff` in `pages/_app.tsx`. For React Web and manual
Next.js provider or root handoff, pass a browser `handoff` built from the server/static/edge state to
the provider or root. A fresh client-side `page()` response or a render boundary can also prevent
...
If a React Native state update omits profile, selected optimizations, or changes, the AsyncStorage
write keeps the cached value for the omitted field instead of clearing that part of profile
continuity.
...
- For App Router Server Components, use response-capable request handling when profile-cookie
  persistence must happen before render; Server Components pass the resulting handoff forward but do
  not write response cookies themselves.
...
- Bootstrap direct Web SDK `defaults`, use the App Router request root or provider for an SDK-owned
  private-request handoff, pass an explicit handoff to top-level App Router selection or manual
  paths, pass Pages Router `pageProps.contentfulOptimization.handoff`, or pass a browser `handoff`
  through React Web or manual Next.js provider or root handoff when server-rendered personalized
  output must match client-side resolution before the first browser Experience response.
...
- [Next.js App Router reference implementation](../../implementations/nextjs-sdk_app-router/README.md)
- [Next.js Pages Router reference implementation](../../implementations/nextjs-sdk_pages-router/README.md)
