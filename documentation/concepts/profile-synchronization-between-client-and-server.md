...
- **Configured defaults** - `defaults.profile`, `defaults.selectedOptimizations`,
  ...
  `NextjsOptimizationState` only as a page-level marker under existing SDK context.
...
- Bootstrap direct Web SDK `defaults`, use React Web or Next.js `serverOptimizationState` for direct
