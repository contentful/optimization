...
- **Configured defaults** - `defaults.profile`, `defaults.selectedOptimizations`,
  ...
  `serverOptimizationState` on the provider or root, which renders it on the server and hydrates it
  on the client. To hydrate page-specific data under an existing provider that was not seeded with
  server data, call `hydrateOptimizationData` from `@contentful/optimization-web/bridge-support`
...
`OptimizationData` to `serverOptimizationState`; the provider renders that state on the server and
hydrates the same data on the client (call `hydrateOptimizationData` from
`@contentful/optimization-web/bridge-support` inside a Client Component to hydrate page-specific
data under an existing provider). A fresh client-side `page()` response or a render boundary can
also prevent stale cached state from driving visible content.
...
- Bootstrap direct Web SDK `defaults`, or use React Web or Next.js `serverOptimizationState` for
  provider handoff (call `hydrateOptimizationData` from
  `@contentful/optimization-web/bridge-support` inside a Client Component for page-level hydration),
  when server-rendered personalized output must match client-side resolution before the first
  browser Experience response.
