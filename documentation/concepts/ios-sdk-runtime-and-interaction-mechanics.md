...
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Configuration        | `spaceId` is required. `environment` defaults to `"master"`. `locale` configures the SDK Experience API and event locale; it does not choose the Contentful CDA locale for app-owned entry fetches.                                                                                                                                                                                                                    |
...
locale, then pass that fetched entry to `OptimizedEntry` or `resolveOptimizedEntry(...)`. The native
iOS SDK does not provide managed entry fetching.
