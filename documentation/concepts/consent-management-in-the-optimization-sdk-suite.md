...
browser handoff. `consent.clientDefaults` seeds browser defaults when configured. Next.js request
handoffs derive browser defaults from the resolved server consent: boolean consent seeds both
consent axes, while object-form consent seeds event consent only when `events` is present and
always seeds persistence consent, using `false` when `persistence` is omitted. Those handoff
defaults override the matching `consent.clientDefaults` axes. Manual
...
lower-level paths when the application owns request binding directly. When the application uses App
Router, Pages Router, or Edge request handoff helpers, configure explicit request consent; omitting
`consent.server` resolves request consent to `false`. When the application policy
permits Optimization by default, bind each request with accepted event and persistence consent:
