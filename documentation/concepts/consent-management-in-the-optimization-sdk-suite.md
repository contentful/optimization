...
- **Allow-list** - The configured `allowedEventTypes` that intentionally can emit while event
  consent is unset or false.
...
| **Next.js**      | App Router `/app-router/server` request components with `consent.server` and browser handoff defaults; `/app-router/client` browser components with `consent.clientDefaults`; Pages Router bindings; advanced App Router `createRequestHandoff(...)`; edge `createEdgeRequestHandoff(...)`; manual server `getNextjsServerOptimizationData(..., { consent })`; and client `OptimizationRoot` | Server and edge paths use application-owned cookies and request state; client paths use React Web storage.               | Server, edge, and client paths follow `identify` and `page` defaults unless `allowedEventTypes` changes. | Server-side `onEventBlocked`; client `onEventBlocked` and `states.blockedEventStream` |
...
- **Persistence consent** - Durable profile continuity is separate from event emission. Boolean
  consent calls set both values by default. JavaScript and React Native object-form consent, plus
  native iOS and Android split-consent overloads, can allow events while keeping profile continuity
  session-only.
...
model through adapter helpers. For App Router integrations, prefer the `/app-router/server` binding,
use its nested request components for private request rendering, and put the request policy in
`consent.server`. For Pages Router `getServerSideProps`, use the `/pages-router/server` binding and
put the same policy in `consent.server`:
...
`{ cookies, headers }`. The App Router request component family resolves it once for the request,
calls the server page path, and supplies the resulting browser handoff to its root or provider.
`consent.clientDefaults` seeds browser defaults when configured. Next.js request handoffs derive
browser defaults from the resolved server consent: boolean consent seeds both consent axes, while
object-form consent seeds event consent only when `events` is present and always seeds persistence
consent, using `false` when `persistence` is omitted. Those handoff defaults override the matching
`consent.clientDefaults` axes. Bound App Router `createRequestHandoff(...)`, manual
`getNextjsServerOptimizationData(..., { consent })`, manual `createNextjsRequestHandoff(...)` with a
`consent` option, response-capable request handlers, and edge `createEdgeRequestHandoff(...)` remain
advanced paths when the application owns request binding directly. When the application uses App
Router request components, Pages Router helpers, or Edge request handoff helpers, configure explicit
request consent; omitting `consent.server` resolves request consent to `false`. When the application
policy permits Optimization by default, bind each request with accepted event and persistence
consent:
...
`allowedEventTypes` is an explicit pre-consent allow-list. Configured event types intentionally emit
...
- Set `allowedEventTypes: []` when no Optimization event can emit before consent; this is the strict
  opt-in posture.
...
- When no pre-consent events are permitted, configure no pre-consent allowed events, such as
  `allowedEventTypes: []` or Android `allowedEventTypes = emptyList()`. Defer Web, React Web, or
  Next.js client initialization when any SDK browser storage access is not permitted before opt-in.
...
- Configure `allowedEventTypes` to match the approved pre-consent event set; use an empty list for
  strict opt-in.
...
- Use JavaScript or React Native `consent({ events, persistence })`, iOS
  ...
  continuity have separate consent choices.
