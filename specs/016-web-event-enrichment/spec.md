# Feature Specification: Contentful Optimization Web Event Enrichment
...
**Independent Test**: Initialize `ContentfulOptimization` with minimal config, emit events, and
verify event-builder defaults for channel/library/locale/page/user-agent are applied.
...
### User Story 2 - Preserve Robust Fallbacks for Page Enrichment Inputs (Priority: P1)

As a maintainer, I need page enrichment helpers to fail safely so event building continues even when
page/global access throws.

**Why this priority**: Browser APIs can fail under restricted runtime conditions.
...
1. **Given** user-supplied event-builder overrides, **When** config merge runs, **Then** defaults
   are preserved unless explicitly overridden (including nested `library` fields).
...
- Locale resolution prefers `navigator.languages[0]` and falls back to `navigator.language`.
- `getPageProperties` fallback payload omits hash/width/height and returns stable empty
  path/search/url/title/referrer/query fields.
- Query extraction stores one value per key based on iteration order.
- Beacon helper returns the browser-provided boolean from `sendBeacon` without retries.
- Build-time constant replacements are optional; fallback constants keep event metadata valid.
...
- **FR-005**: Web config merging MUST combine caller event-builder overrides with defaults via
  object spread semantics, with explicit nested merge for `eventBuilder.library`.
...
- **SC-001**: Default initialization checks confirm web channel/library metadata and enrichers are
...
- **SC-003**: Override checks confirm custom event-builder fields can be applied without dropping
  default Web enrichment.
