...
/// `Contentful.Client(spaceId:accessToken:)` against `cdn.contentful.com`, and
/// every URL the SDK generates is already correct. Two properties of the *mock*
/// server make the shim necessary here:
///
/// - `lib/mocks` multiplexes the Contentful, Experience, and Insights APIs onto
///   one port, namespacing the CDA under `/contentful/`. `Contentful.Client`
///   builds `/spaces/...` from the host root and accepts only a `host[:port]`,
///   so there is nowhere to put the prefix.
/// - The SDK walks locale fallback chains itself, so it fetches `/locales`
///   before the first `/entries` call and refuses to decode entries without the
///   resulting localization context. The mock serves no `/locales` route.
///
/// Installed on the demo client's session only (see ``MockContentfulClient``),
/// never registered globally, so no other networking in the app is affected.
final class MockContentfulTransport: URLProtocol {
