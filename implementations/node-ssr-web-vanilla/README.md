...
4. Start the mock API and application servers:
   ...
   ```sh
   pnpm --dir implementations/node-ssr-web-vanilla --ignore-workspace serve
   ```
...
6. Stop the mock API and application servers:
   ...
   ```sh
   pnpm --dir implementations/node-ssr-web-vanilla --ignore-workspace serve:stop
   ```
...
1. Start servers:
   ...
   ```sh
   pnpm run implementation:node-ssr-web-vanilla -- serve
   ```

2. Stop servers:

   ```sh
   pnpm run implementation:node-ssr-web-vanilla -- serve:stop
   ```

3. Run E2E:

   ```sh
   pnpm run implementation:node-ssr-web-vanilla -- test:e2e
   ```
...
1. Install Playwright dependencies:
   ...
   ```sh
   pnpm --dir implementations/node-ssr-web-vanilla --ignore-workspace exec playwright install --with-deps
   ```

2. Run the E2E test suite:

   ```sh
   pnpm --dir implementations/node-ssr-web-vanilla --ignore-workspace test:e2e
   ```

   The tests can alternatively be run using Playwright's GUI:

   ```sh
   pnpm --dir implementations/node-ssr-web-vanilla --ignore-workspace test:e2e:ui
   ```
