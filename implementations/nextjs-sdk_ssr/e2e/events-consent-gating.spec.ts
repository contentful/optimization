...
const VIEW_DWELL_SETTLE_MS = 1100
...
async function scrollThroughEntries(page: Page, scope: EntryObservationScope): Promise<string[]> {
  ...
  for (const entryTestId of entryTestIds) {
    ...
    await page.waitForTimeout(VIEW_DWELL_SETTLE_MS)
  }
...
test.describe('consent gating', () => {
  ...
  test('allows page events without consent but gates automatic entry view events', async ({
    page,
  }) => {
