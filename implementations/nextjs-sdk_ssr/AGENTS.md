...
- All E2E tests live in `lib/e2e-web`. Hydration-specific specs are gated with `runIf('HYDRATION')`
  and run automatically when `E2E_FLAGS=CSR,HYDRATION`. `SKIP_NO_JS` skips the JavaScript-disabled
  variant resolution suite (`Variant Resolution (SSR, JavaScript disabled)`).
