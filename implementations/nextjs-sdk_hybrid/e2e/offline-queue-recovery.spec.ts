...
async function enableConsent(page: Page): Promise<void> {
  await page.getByTestId('consent-button').click()
  await expect(page.getByTestId('consent-status')).toHaveText('Consent: true')
}

async function clickTrackedHomeEntry(page: Page): Promise<void> {
  const button = page.getByTestId('entry-click-descendant-button').first()
...
test.describe('offline queue and recovery', () => {
  ...
  test('continues tracking Insights API events while offline', async ({ context, page }) => {
    ...
    await clickTrackedHomeEntry(page)
