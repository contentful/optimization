...
export function AnalyticsEventDisplay(): JSX.Element {
  ...
  const visibleEvents = isReady && sdk !== undefined ? events : []
  const visibleRawEventsCount = isReady && sdk !== undefined ? rawEventsCount : 0

  return (
    <section data-testid="analytics-events-container">
      <h2>Analytics Events</h2>
      <p data-testid="events-count">Events: {visibleEvents.length}</p>
      <p data-testid="raw-events-count">Raw Events: {visibleRawEventsCount}</p>
      {visibleEvents.length === 0 ? (
        <p data-testid="no-events-message">No events tracked yet</p>
      ) : null}
