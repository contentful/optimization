...
test.describe('identified user', () => {
  ...
  test.beforeEach(async ({ page, context }) => {
    ...
    // await page.goto('/?userId=someone')
  ...
  test('BACKEND: identifies profile id and associates it with user id', async ({ context }) => {
    const { origins: [origin] } = await context.storageState()
